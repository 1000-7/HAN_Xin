...
with open('/Users/unclewang/master/graduate/data/section3500', 'r') as f:
    for line in f:
        rows = line.split("\t")
        words = word_tokenizer.tokenize(rows[2])  # 将评论句子按单词切割
        for word in words:
            word_freq[word] += 1
    print("load finished")
#
...
with open('/Users/unclewang/master/graduate/data/section3500', 'r') as f:
    for line in f:
        # doc = []
        doc = np.zeros((20, 30), dtype=np.int32)
        rows = line.split("\t")
        sents = sent_tokenizer.tokenize(rows[2])  # 将评论分句
    ...
    pickle.dump((data_x, data_y), open('min_data', 'wb'))
