...
class NNfeature(object):
    def __init__(self, data_path='./data/context'):
        ...
        self.one_hot_label = np.zeros(shape=(len(self.label), class_num))
...
class DynamicRnnfeature(object):
    def __init__(self, data_path='./data/context'):
        logger.info('fastTextfeature loading corpus ...')
        self.label_list = ['Military', 'Economy', 'Culture', 'Sports', 'Auto', 'Medicine']
        ...
        self.train_x, self.test_x, self.train_y, self.test_y, self.seq_len_train, self.seq_len_test = \
            train_test_split(self.context, self.one_hot_label, self.sequence_length, test_size=0.05)
        logger.debug('self.train_x shape: (%d, %d)' % (self.train_x.shape))
        logger.debug('self.test_x shape: (%d, %d)' % (self.test_x.shape))
        logger.debug('self.train_y shape: (%d, %d)' % (self.train_y.shape))
        logger.debug('self.test_y shape: (%d, %d)' % (self.test_y.shape))
        logger.debug('self.seq_len_train shape: (%d)' % (self.seq_len_train.shape))
        logger.debug('self.seq_len_test shape: (%d)' % (self.seq_len_test.shape))
